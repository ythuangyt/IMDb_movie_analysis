...
# In[]
## functions to pridict signals
#&&&&&&&&&&&    
#def actor_vector(vector, features):
#    print('---> Generating weight matrix of actor/actress ...')
#    # get cast_name for each movie
#    cast_name = []
#    for index,i in zip(features.index, features['cast']):
...
#
#    # compute distance of average votes between each nodes
#    Dis_vote[:,1] = np.abs(vote_avg.values - value)
#
#    # calculate the Wright matrix of the IMDb vote graph
#    sigma  = np.nanstd(Dis_vote)
#    V_vote = np.exp( (-Dis_vote**2)/(sigma*sigma) )
#    V_vote = V_vote - np.diag(np.ones(N_node))
...
#
#    # compute distance of average votes between each nodes
#    Dis_budg[:,1] = np.abs(budget - value)
#
#    # calculate the Wright matrix of the IMDb vote graph
#    sigma  = np.nanmean(Dis_budg)*1.5
#    V_budg = np.exp( (-Dis_budg**2)/(sigma*sigma) )
#    V_budg = V_budg - np.diag(np.ones(N_node))
#    print('---> Weight matrix of votes has been generated")
#    return V_budg
#
#
#def director_vector(File_path, features):
#&&&&&&&&&&&&&&8 
#    crew_name = []
#    for item in features.crew:
#        tmp2 = []
...
#
#    np.save(File_path + '/weight_mat_director', W_dire)
#    print('---> Weight matrix of directors has been generated')
#    return W_dire
#
#
#def keyword_vector(File_path, features):
#&&&&&&&&&&&&&9  
#    # ---------- Generate sentence list for training Word2Vec model ---------- #
#    features_keywords = features['keywords'].map(lambda x: [item['name'] for item in x])
#    features_keywords = features_keywords.map(lambda x: [re.sub('[^A-Za-z0-9_]+', '_', item) for item in x if len(x)!=0])
...
#    W_keyw = generate_adja(generate_dist(bag_of_keywords))
#
#    print('---> Weight matrix of keywords has been generated')
#    return W_keyw
#
#
...
#
#    # compute distance of average votes between each nodes
#    Dis_pop[:,1] = np.abs(popularity - value)
#
#    # calculate the Wright matrix of the IMDb vote graph
#    sigma  = Dis_pop.mean()
#    V_pop = np.exp( (-Dis_pop**2)/(sigma*sigma) )
#    V_pop = V_pop - np.diag(np.ones(N_node))
#    print("---> Weight matrix of votes has been generated")
#    return V_pop
#
#
...
#        if feature == 'cast':
#            weight_vec  = actor_vector ( value[ct_value], features )
#        elif feature == 'vote_average':
#            weight_vec  = vote_vector  ( value[ct_value], features )
#        elif feature == 'budget':
#            weight_vec  = budget_vector( value[ct_value], features )
#        elif feature == 'crew':
#            weight_vec  = director_vector(value[ct_value], features)
#        elif feature == 'genres':
#            print("Since we separate the whole in different genres, we won't use genre as a feature")
#            continue
#        elif feature == 'keywords':
#            weight_vec  = keyword_vector(value[ct_value], features )
##            weight_vec = advkeyw_graph( path_to_file, features )
#        elif feature == 'popularity':
